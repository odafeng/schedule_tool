        ...
        try:
            ...
            if os.path.exists(temp_file):
                if os.path.exists(final_file):
                    ...
                    backup_files = sorted([f for f in os.listdir(backup_dir) if f.startswith('doctors.json.backup_')])
            ...
            print(f"成功儲存 {len(doctors_list)} 位醫師資料 (年份: {year}, 月份: {month})")
            return True
        ...
        try:
            ...
            for doctor_dict in doctors_list:
                ...
                for date_item in unavailable_dates:
                    if isinstance(date_item, str) and re.match(r'^\d{4}-\d{2}-\d{2}$', date_item):
                        ...
                        if 1 <= day <= 31:
                            try:
                                ...
                                processed_unavailable.append(date_obj.strftime("%Y-%m-%d"))
                            except ValueError:
                                # 無效的日期（如2月31日）
                                continue
                    ...
                    if isinstance(date_item, str) and re.match(r'^\d{4}-\d{2}-\d{2}$', date_item):
                        ...
                        if 1 <= day <= 31:
                            try:
                                from datetime import date
                                date_obj = date(year, month, day)
                                processed_preferred.append(date_obj.strftime("%Y-%m-%d"))
                            except ValueError:
                                # 無效的日期（如2月31日）
                                continue
            ...
            for doctor in st.session_state.doctors[:3]:
                if doctor.unavailable_dates:
                    print(f"{doctor.name}: 不可值班日 = {doctor.unavailable_dates[:3]}")
            print("=" * 50)
