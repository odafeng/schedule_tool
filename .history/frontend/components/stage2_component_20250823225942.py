...
def render_auto_fill_tab_cli(swapper):
    ...
    if st.session_state.auto_fill_results is not None:
        ...
        if st.session_state.execution_logs:
            with st.expander("📜 執行日誌", expanded=False):
                # 格式化日誌顯示
                log_text = "\n".join(st.session_state.execution_logs[-100:])  # 顯示最後100行
                st.text_area("系統執行日誌", value=log_text, height=400, disabled=True)
    ...
    if st.button("🚀 開始智慧填補", type="primary", use_container_width=True, 
        ...
        with info_container:
            ...
            - 最長搜索時間：2 分鐘
            - 交換鏈深度：5 步
            - 空缺數量：{report['summary']['unfilled_slots']} 個
            """)
        ...
        def log_callback(message: str, level: str = "info"):
            ...
            if current_time - last_update_time < 0.5 and "完成" not in message:
                return
            ...
            if "開始處理" in message:
                progress = 0.1
                status_container.info("🔄 正在初始化系統...")
            elif "直接填補" in message:
                progress = 0.3
                status_container.info("✅ 正在執行直接填補...")
            elif "開始深度搜索" in message:
                progress = 0.5
                status_container.warning("🔍 正在執行深度搜索...")
            elif "找到" in message and "可行方案" in message:
                progress = 0.7
                status_container.info("✨ 正在評估交換方案...")
            ...
            progress_text.text(f"進度：{int(progress * 100)}%")
            ...
            **執行統計**
            - ⏱️ 已用時間：{elapsed:.1f} 秒
            - 📊 日誌條數：{len(log_messages)}
            - 🔄 當前狀態：{'執行中...' if progress < 1.0 else '已完成'}
            """)
            ...
            for ts, msg, lvl in display_logs:
                ...
                log_lines.append(f"{emoji} [{ts}] {msg}")
            
            # 使用 text_area 顯示日誌
            log_text = "\n".join(log_lines)
            log_container.text_area("", value=log_text, height=400, key=f"log_{current_time}")
        ...
        try:
            ...
            - 已填補：{len(results['direct_fills']) + len(results['swap_chains'])} 個
            - 剩餘：{len(results['remaining_gaps'])} 個
            """)
