...
def render_stage2_advanced(weekdays: list, holidays: list):
    ...
    if "stage2_mode" not in st.session_state:
        st.session_state.stage2_mode = "calendar"  # calendar, filling, output
...
def render_final_output_content(swapper, weekdays: list, holidays: list):
    ...
    for week_num, week in enumerate(cal):
        ...
        for day_num, day in enumerate(week):
            ...
            with cols[day_num]:
                ...
                # 格子樣式
                bg_color = "#fee2e2" if is_holiday else "#e0e7ff"
                
                with st.container():
                    # 日期標題
                    st.markdown(f"""
                    <div style='background: {bg_color}; padding: 5px; border-radius: 5px 5px 0 0; text-align: center;'>
                        <b>{day}日</b> {'🎉' if is_holiday else ''}
                    </div>
                    """, unsafe_allow_html=True)
    ...
    with col3:
        if st.button("🔄 重置變更", use_container_width=True):
            # 重新載入原始 schedule
