...
def render_auto_fill_tab_fixed(swapper):
    ...
    if st.session_state.auto_fill_logs:
        ...
        with st.expander("查看詳細日誌", expanded=True):
            ...
            # 創建黑色背景的終端機風格日誌顯示區
            log_text = "\n".join(st.session_state.auto_fill_logs[-100:])  # 只顯示最後100條
            ...
            st.markdown(
                ...
                ">
                    <pre style="color: #ffffff; margin: 0;">{log_text}</pre>
                </div>
                """,
...
def execute_auto_fill_safe(swapper, log_container, progress_bar, status_text):
    ...
    def add_log(message: str, level: str = "INFO"):
        ...
        should_update = (
        len(st.session_state.auto_fill_logs) % 10 == 0 or  # 每 10 條更新
        level in ["SUCCESS", "ERROR", "WARNING"] or  # 重要訊息立即更新
        "完成" in message or "失敗" in message  # 關鍵字立即更新
        )
        ...
        if should_update:
            ...
            <div style="
                ...
                border: 1px solid #333333;
            ">
                <pre style="color: #ffffff; margin: 0;">{"<br>".join(recent_logs)}</pre>
            </div>
            """
            ...
            if "處理空缺" in message:
                ...
                # 更新進度條
                progress = min(0.1 + (search_metrics["backtracks"] / max_backtracks) * 0.8, 0.9)
                progress_bar.progress(progress)
                status_text.text(f"執行中... (回溯: {search_metrics['backtracks']}/{max_backtracks})")
