...
def render_stage2_advanced(weekdays: list, holidays: list):
    ...
    with tab2:
        render_auto_fill_tab_simple(swapper)
...
def render_auto_fill_tab_simple(swapper):
    """智慧填補標籤頁（簡化版）"""
    ...
    if st.button("🚀 開始智慧填補", type="primary", use_container_width=True,
        ...
        # 執行填補
        execute_auto_fill(swapper, report)


def execute_auto_fill(swapper, report):
    """執行自動填補"""
    ...
    def log_callback(message: str, level: str = "info"):
        ...
        # 更新日誌顯示
        log_text = "\n".join(logs_collected[-10:])  # 顯示最後10行
    ...
    with progress_placeholder.container():
        st.info(f"🔄 正在執行智慧填補... (最多 {max_backtracks:,} 次回溯)")
...
def display_results(results):
    ...
    if st.session_state.execution_logs:
        with st.expander("📜 執行日誌", expanded=False):
            log_text = "\n".join(st.session_state.execution_logs[-100:])
