    ...
    if st.button("🚀 開始智慧填補", type="primary", use_container_width=True, 
        ...
        def log_callback(message: str, level: str = "info"):
            ...
            if "開始深度搜索" in message:
                progress_bar.progress(0.3, text="執行深度搜索...")
                status_text.info(f"🔍 {message}")
            elif "找到" in message and "可行方案" in message:
                progress_bar.progress(0.5, text="評估交換方案...")
                status_text.success(f"✅ {message}")
            elif "直接填補" in message:
                progress_bar.progress(0.7, text="執行直接填補...")
                status_text.success(f"✅ {message}")
        ...
        with st.spinner("🔄 正在執行智慧填補，請稍候..."):
            try:
                ...
                progress_bar.progress(1.0, text="✅ 處理完成！")
                if results['remaining_gaps']:
                    status_text.warning(f"⚠️ 執行完成，但還有 {len(results['remaining_gaps'])} 個空缺未解決")
                else:
                    status_text.success("🎉 完美執行！所有空缺已成功填補")
                ...
                progress_bar.progress(0, text="執行失敗")
