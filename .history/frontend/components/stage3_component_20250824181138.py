...
def render_stage3(weekdays: list, holidays: list):
    ...
    with col3:
        if st.button("✅ 確認完成", type="primary", use_container_width=True):
            # 使用 SessionManager 更新最終班表
            from frontend.utils.session_manager import SessionManager
            
            if st.session_state.stage3_publisher:
                SessionManager.update_final_schedule(
                    st.session_state.stage3_publisher.schedule,
                    source_stage='stage3'
                )
                ...
                with col_a:
                    if st.button("📊 前往檢視班表", use_container_width=True):
                        st.session_state.page = "schedule_viewer"  # 假設你有頁面導航
                        st.rerun()
                
                with col_b:
                    if st.button("🔄 開始新的排班", use_container_width=True):
                        SessionManager.clear_all_schedules()
                        st.rerun()
...
def render_export_section(publisher, weekdays, holidays):
    """匯出與發佈區塊"""
    from backend.utils.supabase_client import get_supabase_manager
    from backend.utils.linebot_client import get_line_bot_client, format_schedule_message
    ...
    with col2:
        ...
        manager = get_supabase_manager()
        if manager.get_status()['connected']:
            if st.button("📤 生成並上傳 PDF", use_container_width=True):
                with st.spinner("生成 PDF 並上傳中..."):
                    try:
                        ...
                        if download_url:
                            ...
                            st.session_state.last_download_url = download_url
                            
                            # 顯示下載連結
                            with st.expander("📥 下載連結"):
    ...
    if line_client:
        ...
        with col3:
            ...
            send_mode = st.radio(
                ...
                horizontal=True,
                key="line_send_mode",
                help="Flex Message 提供更豐富的視覺效果"
            )
            
            # 預覽訊息
            with st.expander("預覽訊息內容", expanded=False):
                # 準備統計資料
                statistics = {
                    'doctor_duties': publisher.quality_report.statistics.get('doctor_duties', {}),
                    'total_days': len(publisher.schedule),
                    'doctor_count': len(publisher.doctors),
                    'weekday_count': len(weekdays),
                    'holiday_count': len(holidays)
                }
                
                # 取得下載連結
                download_url = st.session_state.get('last_download_url', None)
                
                if send_mode == "📝 純文字":
                    preview_message = format_schedule_message(
                    ...
                    st.info("Flex Message 將以圖文格式呈現統計資料")
        
        with col4:
            st.markdown("##### 發送操作")
            if st.button("📤 發送到 LINE", use_container_width=True, type="primary"):
                with st.spinner("發送中..."):
                    try:
                        # 準備統計資料
                        statistics = {
                        ...
                        if send_mode == "📝 純文字":
                            ...
                            # 發送訊息
                            result = line_client.broadcast_message(message)
                        else:
                            # 建立並發送 Flex Message
                            flex_message = line_client.create_schedule_flex_message(
                                year=st.session_state.selected_year,
                                month=st.session_state.selected_month,
                                statistics=statistics,
                                download_url=download_url
                            ...
                            result = line_client.send_flex_message(flex_message)
                        ...
                        if result['success']:
                            ...
                            # 儲存發送記錄
                            save_line_publish_record(result)
                        else:
                            st.error(f"❌ 發送失敗：{result['message']}")
                            
                    except Exception as e:
                        st.error(f"發送錯誤：{str(e)}")
            
            # 測試連線
            if st.button("🔧 測試連線", key="test_line"):
                if line_client.test_connection():
                    st.success("✅ 連線正常")
                else:
                    st.error("❌ 連線失敗")
    else:
        st.warning("⚠️ LINE Bot 未設定")
        with st.expander("查看設定說明", expanded=False):
            st.markdown("""
            ### 🔧 LINE Bot 已內建
            
            系統已內建 LINE Bot Token，可直接使用。
            
            如需更換 Token，請：
            1. 建立 `.env` 檔案
            2. 加入 `LINE_CHANNEL_ACCESS_TOKEN=你的TOKEN`
            3. 重新啟動應用程式
            """)


def save_line_publish_record(result: Dict):
    """儲存 LINE 發送紀錄"""
    record = {
    ...
    os.makedirs("data/line_history", exist_ok=True)
    filename = f"data/line_history/{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
