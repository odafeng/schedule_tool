...
def execute_auto_fill_safe(swapper, log_container, progress_bar, status_text):
    ...
    max_backtracks = 10000  # 減少回溯次數避免超時
    ...
    def add_log(message: str, level: str = "INFO"):
        ...
        if current_gap_count != search_metrics["last_gap_count"]:
            if current_gap_count < search_metrics["last_gap_count"]:
                add_log(f"📉 空缺減少: {search_metrics['last_gap_count']} → {current_gap_count}", "SUCCESS")
            else:
                add_log(f"📈 空缺增加: {search_metrics['last_gap_count']} → {current_gap_count} (可能因回溯)", "WARNING")
