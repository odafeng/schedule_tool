...
def render_auto_fill_tab_cli(swapper):
    ...
    if st.button("🚀 開始智慧填補", type="primary", use_container_width=True, 
        ...
        # 使用 st.status 來顯示執行狀態（這是 Streamlit 原生支援的）
        with st.status("🚀 正在執行智慧填補...", expanded=True) as status:
            ...
            def update_display(elapsed_time, progress_value, log_messages):
                """更新顯示內容"""
                # 更新計時器
                timer_container.markdown(f"""
                **⏱️ 執行時間：{elapsed_time:.1f} 秒**
                """)
            ...
            def log_callback(message: str, level: str = "info"):
                ...
                update_display(elapsed, progress, log_messages)
            ...
            log_callback(f"最大回溯次數設定: {max_backtracks:,}", "info")
