...
def render_stage3(weekdays: list, holidays: list):
    ...
    if "supabase_client" not in st.session_state:
        st.warning("⚠️ 請先設定 Supabase 連線")
        with st.expander("Supabase 設定", expanded=True):
        ...
        if st.button("📤 生成並上傳 PDF", use_container_width=True, type="primary"):
            with st.spinner("生成 PDF 並上傳中..."):
                try:
                    ...
                    # 上傳到 Supabase
                    download_url = upload_to_supabase(pdf_filename)
                    ...
                    if download_url:
                        ...
                        st.button(
                            "📋 複製連結",
                            on_click=lambda: st.write(download_url),
                            use_container_width=True
                        )
