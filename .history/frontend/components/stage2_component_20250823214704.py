...
def render_auto_fill_tab_cli(swapper):
    ...
    if st.session_state.auto_fill_results is not None:
        ...
        if results['remaining_gaps']:
            ...
            - ⏱️ 總耗時：{results.get('elapsed_time', 0):.2f} 秒
            """)
        ...
        if st.session_state.cli_logs:
            with st.expander("📜 執行日誌", expanded=False):
                # 創建 CLI 風格的日誌顯示
                log_html = create_cli_log_html(st.session_state.cli_logs)
    ...
    if st.button("🚀 開始智慧填補", type="primary", use_container_width=True, 
        ...
        def log_callback(message: str, level: str = "info"):
            ...
            st.session_state.cli_logs.append(log_entry)
            
            # 更新 CLI 顯示
            html_content = create_cli_log_html(st.session_state.cli_logs[-50:])  # 顯示最近50條
            log_container.markdown(
                f'<div class="terminal-container">{html_content}</div>',
                unsafe_allow_html=True
            )
            
            # 更新進度條和時間
            elapsed = time.time() - start_time
            elapsed_time_display.metric("已用時間", f"{elapsed:.1f} 秒")
            
            # 估算剩餘時間（基於空缺數量）
            if report['summary']['unfilled_slots'] > 0:
                avg_time_per_gap = 3  # 假設每個空缺平均需要3秒
                estimated_total = report['summary']['unfilled_slots'] * avg_time_per_gap
                estimated_remaining = max(0, estimated_total - elapsed)
                estimated_time_display.metric("預估剩餘", f"{estimated_remaining:.0f} 秒")
                
                # 更新進度條
                if estimated_total > 0:
                    progress = min(1.0, elapsed / estimated_total)
