...
def render_auto_fill_tab_realtime(swapper):
    ...
    if "auto_fill_stop_flag" not in st.session_state:
        st.session_state.auto_fill_stop_flag = {"stop": False}
        ...
        if not st.session_state.auto_fill_running:
            if st.button("🚀 開始智慧填補", type="primary", use_container_width=True):
                ...
                thread = threading.Thread(
                    ...
                    args=(swapper, logger, st.session_state.auto_fill_stop_flag),
                    daemon=True
                )
                thread.start()
            ...
            if st.button("⏹️ 停止執行", use_container_width=True):
                st.session_state.auto_fill_stop_flag["stop"] = True
                st.session_state.auto_fill_running = False
...
def execute_auto_fill_realtime(swapper, logger: RealTimeLogger, stop_flag=None):
    """在背景執行緒中執行自動填補，並即時記錄日誌"""
    max_backtracks = 20000
    
    # 使用一個共享的字典來儲存進度，避免直接訪問 session_state
    ...
    def realtime_log_callback(message: str, level: str = "info"):
        ...
        if "直接填補成功" in message:
            ...
            # 嘗試更新 session state（如果可用）
            try:
                if hasattr(st, 'session_state') and 'auto_fill_progress' in st.session_state:
        ...
        # 檢查是否應該停止（使用 stop_flag 而非 session_state）
