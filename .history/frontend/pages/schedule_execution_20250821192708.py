...
from backend.algorithms.stage2_interactive import Stage2InteractiveFiller
    ...
    **策略**：
    ...
    with col1:
        beam_width = st.slider("束搜索寬度", 3, 10, 5,
                              help="保留的候選解數量，越大越精確但越慢")
    
    with col2:
        st.metric("預計填充率", "85-95%")
    
    with col3:
        st.metric("預計時間", f"{len(weekdays + holidays) * 0.1:.0f} 秒")
...
def execute_stage1(weekdays: list, holidays: list, beam_width: int):
    ...
    with col1:
        if st.button("👁️ 預覽選中方案", use_container_width=True):
            preview_schedule(results[selected_index].schedule)
    
    with col2:
        if st.button("✅ 採納並進入 Stage 2", type="primary", use_container_width=True):
    ...
    for gap in gaps_to_show:
        with st.expander(
            ...
            for i, candidate in enumerate(candidates[:5]):
                ...
                with col3:
                    feasibility_color = "🟢" if candidate.feasibility > 0.7 else "🟡" if candidate.feasibility > 0.3 else "🔴"
                    st.text(f"{feasibility_color} {candidate.feasibility:.0%}")
                
                with col4:
                    if st.button("選擇", key=f"select_{gap.date}_{gap.role}_{candidate.name}"):
...
def render_swap_suggestions(filler: Stage2InteractiveFiller):
    ...
    for i, suggestion in enumerate(suggestions):
        with st.expander(f"建議 {i+1}: {suggestion.description} (改善 +{suggestion.score_improvement:.0f}分)"):
            col1, col2 = st.columns(2)
...
def render_stage3(weekdays: list, holidays: list):
    ...
    if report.critical_issues or report.minor_issues:
        ...
        if report.critical_issues:
            with st.expander(f"🔴 重要問題 ({len(report.critical_issues)})", expanded=True):
                for issue in report.critical_issues:
                    st.error(f"• {issue}")
        
        if report.minor_issues:
            with st.expander(f"🟡 次要問題 ({len(report.minor_issues)})", expanded=False):
