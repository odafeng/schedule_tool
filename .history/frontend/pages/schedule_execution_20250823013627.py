...
from backend.algorithms.stage2_interactiveCSP import Stage2AdvancedSwapper  # 使用新的類別
...
def render_swap_exploration_tab(swapper: Stage2AdvancedSwapper):
    ...
    if selected_gap_idx is not None:
        ...
        if st.button("🔍 搜索交換鏈", use_container_width=True):
            with st.spinner("正在搜索可行的交換鏈..."):
                chains = swapper.find_multi_step_swap_chains(gap, max_depth)
                
                if chains:
                    st.success(f"找到 {len(chains)} 個可行交換鏈")
                    
                    # 顯示前 5 個方案
                    for i, chain in enumerate(chains[:5]):
                        with st.expander(f"方案 {i+1} (總分: {chain.total_score:.1f})"):
