...
def execute_stage1(weekdays: list, holidays: list, beam_width: int):
    ...
    selected_index = st.radio(
        ...
        key="solution_selection",  # 添加明確的key
    )

    col1, col2 = st.columns(2)

    with col1:
        if st.button("👁️ 預覽選中方案", key="preview_btn"):
            with st.expander("排班預覽", expanded=True):
                preview_schedule(results[selected_index].schedule)

    with col2:
        if st.button("✅ 採用並進入 Stage 2", type="primary", key="adopt_btn"):
