    ...
    with col1:
        if st.button("📂 載入上次設定", type="secondary", use_container_width=True):
            if SessionManager.load_doctors():
                # 驗證載入的資料
                is_valid, errors = SessionManager.validate_loaded_dates()
                
                if not is_valid:
                    # 顯示錯誤並提供修正選項
                    st.warning("偵測到日期格式問題")
                    ...
                    if len(errors) > 5:
                        st.error(f"... 還有 {len(errors) - 5} 個錯誤")
                    
                    # 提供自動修正選項
                    col_a, col_b = st.columns(2)
                    with col_a:
                        if st.button("🔧 自動修正（移除無效日期）", type="primary"):
                            fixed = SessionManager.fix_date_issues()
                            st.success(f"✅ 已修正 {fixed} 個日期問題")
                            # 自動儲存修正後的資料
                            SessionManager.save_doctors()
                            st.rerun()
                    
                    with col_b:
                        if st.button("❌ 取消載入", type="secondary"):
