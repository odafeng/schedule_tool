...
def render_stage1(weekdays: list, holidays: list):
    ...
    if st.session_state.stage1_results is not None:
        ...
        st.success(f"✅ Stage 1 已完成！產生了 {len(results)} 個候選方案")
        ...
        if st.button("🚀 開始 Stage 1 排班", type="primary", use_container_width=True):
            ...
            with col1:
                st.write("**平日格式檢查：**")
                st.write(f"數量: {len(weekdays)}")
                if weekdays:
                    st.write("前3個平日：")
                    for i, date in enumerate(weekdays[:3]):
                        st.code(f"[{i}] = '{date}' (類型: {type(date).__name__}, 長度: {len(date)})")
            ...
            for doctor in st.session_state.doctors[:3]:
                unavail_sample = doctor.unavailable_dates[:2] if doctor.unavailable_dates else []
                prefer_sample = doctor.preferred_dates[:2] if doctor.preferred_dates else []
                
                test_data.append({
                    "醫師": doctor.name,
                    "角色": doctor.role,
                    "不可值班日數": len(doctor.unavailable_dates),
                    "不可值班範例": str(unavail_sample),
                    "優先值班日數": len(doctor.preferred_dates),
                    "優先值班範例": str(prefer_sample)
                })
            ...
            for doctor in st.session_state.doctors:
                if doctor.unavailable_dates:
                    ...
                    st.write(f"**測試案例：{doctor.name}**")
                    st.write(f"- 不可值班日: {test_date}")
                    st.write(f"- 是否在平日列表: {test_date in weekdays}")
                    st.write(f"- 是否在假日列表: {test_date in holidays}")
                    
                    # 詳細比對
                    if test_date in weekdays or test_date in holidays:
                        st.success(f"✅ 約束日期 '{test_date}' 在排班日期中，會被正確識別")
                    else:
                        st.error(f"❌ 約束日期 '{test_date}' 不在本月排班日期中")
                    
                    # 顯示字串比對細節
                    with st.expander("查看詳細比對"):
                        st.write(f"測試日期: '{test_date}'")
                        st.write(f"測試日期長度: {len(test_date)}")
                        st.write(f"測試日期類型: {type(test_date)}")
                        
                        if weekdays:
                            st.write(f"第一個平日: '{weekdays[0]}'")
                            st.write(f"第一個平日長度: {len(weekdays[0])}")
                            st.write(f"是否相等: {test_date == weekdays[0]}")
                    break
            ...
            # 顯示將要執行的參數
            st.write(f"- 平日數: {len(weekdays)}")
            st.write(f"- 假日數: {len(holidays)}")
            st.write(f"- 醫師數: {len(st.session_state.doctors)}")
            st.write(f"- 束寬度: {constraints.beam_width}")
            
            # 確認執行
            if st.button("確認執行（已看過調試資訊）", type="primary"):
...
def preview_schedule_inline(schedule: dict):
    ...
    with st.container():
        ...
        with col3:
            st.metric(
                "總填充率", f"{(filled_attending + filled_resident)/(total*2):.1%}"
            )
...
def execute_stage1(weekdays: list, holidays: list, beam_width: int):
    ...
    # ========== 重要：在 spinner 外面顯示調試資訊 ==========
    st.write("---")
    st.write("## 🔍 調試資訊")
    
    # 顯示日期格式
    with st.expander("📅 日期格式檢查", expanded=True):
        col1, col2 = st.columns(2)
        
        with col1:
            st.write("**傳入的日期格式：**")
            st.write(f"平日數量: {len(weekdays)}")
            if weekdays:
                st.write("前3個平日:")
                for date in weekdays[:3]:
                    st.code(f"'{date}'")
            
            st.write(f"假日數量: {len(holidays)}")
            if holidays:
                st.write("所有假日:")
        ...
        with col2:
            st.write("**醫師約束格式：**")
            for doctor in st.session_state.doctors[:2]:
    ...
    # 測試約束檢查
    with st.expander("🧪 約束檢查測試", expanded=True):
        st.write("**測試不可值班日是否正確識別：**")
        
        # 找一個有不可值班日的醫師來測試
        test_performed = False
        for doctor in st.session_state.doctors:
            if doctor.unavailable_dates and len(doctor.unavailable_dates) > 0:
                test_date = doctor.unavailable_dates[0]
                
                st.write(f"測試醫師: {doctor.name}")
                st.write(f"測試日期: '{test_date}'")
                st.write(f"醫師的不可值班日列表: {doctor.unavailable_dates}")
                
                # 檢查該日期是否在排班日期中
                if test_date in weekdays:
                    st.write(f"✅ '{test_date}' 在平日列表中")
                elif test_date in holidays:
                    st.write(f"✅ '{test_date}' 在假日列表中")
                else:
                    st.warning(f"⚠️ '{test_date}' 不在本月排班日期中")
                
                # 測試直接比對
                st.write(f"\n直接比對測試:")
                st.write(f"'{test_date}' in {doctor.unavailable_dates[:3]}...")
                st.write(f"結果: {test_date in doctor.unavailable_dates}")
                
                # 逐個字元比對（深度調試）
                if len(doctor.unavailable_dates) > 0:
                    first_unavail = doctor.unavailable_dates[0]
                    st.write(f"\n字元層級比對:")
                    st.write(f"測試日期長度: {len(test_date)}")
                    st.write(f"第一個不可值班日長度: {len(first_unavail)}")
                    st.write(f"是否相等: {test_date == first_unavail}")
                    
                    # 顯示字元編碼
                    st.write(f"測試日期編碼: {[ord(c) for c in test_date[:10]]}")
                    st.write(f"不可值班日編碼: {[ord(c) for c in first_unavail[:10]]}")
    ...
    # 顯示格式一致性檢查
    with st.expander("✅ 格式一致性檢查", expanded=False):
        issues = []
        
        # 檢查日期格式是否都是 YYYY-MM-DD
        ...
        for date in weekdays + holidays:
            if not check_format(date):
                issues.append(f"排班日期格式異常: '{date}'")
        
        # 檢查醫師約束
        for doctor in st.session_state.doctors:
            for date in doctor.unavailable_dates:
                if not check_format(date):
                    issues.append(f"{doctor.name} 不可值班日格式異常: '{date}'")
            for date in doctor.preferred_dates:
                if not check_format(date):
                    issues.append(f"{doctor.name} 優先值班日格式異常: '{date}'")
        
        if issues:
            st.error(f"發現 {len(issues)} 個格式問題:")
            for issue in issues[:10]:
                st.write(f"- {issue}")
        else:
            st.success("✅ 所有日期格式正確 (YYYY-MM-DD)")
    ...
    total_violations = 0
    for i, state in enumerate(results):
        violations = scheduler._validate_hard_constraints(state.schedule)
        if violations:
            st.error(f"方案 {i+1} 有 {len(violations)} 個違規:")
            for v in violations[:3]:
                st.write(f"  - {v}")
            total_violations += len(violations)
    
    if total_violations > 0:
        st.error(f"⚠️ 總共發現 {total_violations} 個硬約束違規！")
    else:
        st.success("✅ 所有方案都符合硬約束")
    ...
    st.success(f"✅ Stage 1 完成！產生了 {len(results)} 個候選方案")
