...
def render_single_day_adjustment():
    ...
    all_dates = sorted(weekdays_list + holidays_list)
...
def render_export_section(scheduler, weekdays, holidays):
    ...
    # 使用調整後的班表，安全地處理 None 值
    final_schedule = None
    
    # 優先使用 adjusted_schedule
    if 'adjusted_schedule' in st.session_state and st.session_state.adjusted_schedule:
        final_schedule = st.session_state.adjusted_schedule
    # 如果沒有 adjusted_schedule，嘗試從 schedule_result 取得
    elif 'schedule_result' in st.session_state and st.session_state.schedule_result:
        if hasattr(st.session_state.schedule_result, 'schedule'):
            final_schedule = st.session_state.schedule_result.schedule
    # 最後嘗試從 scheduler 取得
    elif scheduler and hasattr(scheduler, 'schedule'):
    ...
    with col1:
        ...
        if st.button("產生 Excel 檔案", use_container_width=True):
            try:
                ...
                # 檢查 exporter 是否有這些方法
                if hasattr(exporter, '_create_calendar_sheet'):
                    exporter._create_calendar_sheet(wb)
                if hasattr(exporter, '_create_statistics_sheet'):
                    exporter._create_statistics_sheet(wb)
                if hasattr(exporter, '_create_doctors_sheet'):
...
def render_list_view(result, scheduler, weekdays, holidays):
    ...
    # 確保 weekdays 和 holidays 傳遞正確的類型
    df_schedule = schedule_table.create_dataframe(
        display_schedule,
        scheduler,
        weekdays,  # create_dataframe 會自己處理 set/list 轉換
        holidays   # create_dataframe 會自己處理 set/list 轉換
    )
