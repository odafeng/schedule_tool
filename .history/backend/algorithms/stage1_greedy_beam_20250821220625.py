...
class Stage1Scheduler:
    ...
    def _greedy_initialization(self) -> List[SchedulingState]:
        ...
        # 策略1: 清空不同比例的格子重新填充
        for i in range(1, min(3, self.constraints.beam_width)):
        ...
        # 策略2: 如果還需要更多變體，使用更激進的方法
        while len(states) < min(5, self.constraints.beam_width):
            # 不使用固定種子，每次都不同
            variant = self._create_alternative_variant(schedule, used_quota, len(states) + 10)
            if variant:
                variant_state = self._create_state(variant)
                states.append(variant_state)
