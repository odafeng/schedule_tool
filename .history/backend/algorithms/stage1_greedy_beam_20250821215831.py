...
class Stage1Scheduler:
    ...
    def _greedy_initialization(self) -> List[SchedulingState]:
        ...
        # 產生多個變體
        states = [base_state]
        max_attempts = 20  # 增加嘗試次數
        
        for i in range(max_attempts):
            if len(states) >= min(self.constraints.beam_width, 5):
                break
            
            variant = self._create_alternative_variant(schedule, used_quota, i)
            if variant:
                ...
                for existing in states:
                    if self._are_schedules_identical(variant_state.schedule, existing.schedule):
                        is_different = False
                        break
        ...
        while len(states) < min(3, self.constraints.beam_width):
            variant = self._create_radical_variant(schedule, used_quota, attempt_count)
            if variant:
                variant_state = self._create_state(variant)
                # 不檢查是否相同，直接加入
                states.append(variant_state)
            attempt_count += 1
            if attempt_count > 50:  # 防止無限循環
                break
        
        return states
    
    def _are_schedules_identical(self, schedule1: Dict, schedule2: Dict) -> bool:
        """檢查兩個排班表是否完全相同"""
        for date in schedule1:
            if date not in schedule2:
                return False
            if schedule1[date].attending != schedule2[date].attending:
                return False
            if schedule1[date].resident != schedule2[date].resident:
                return False
        return True
    
    def _create_alternative_variant(self, base_schedule: Dict, used_quota: Dict, attempt: int) -> Optional[Dict]:
        """創建替代變體（使用更激進的策略）"""
        # 使用動態種子，結合時間戳和attempt
        random.seed(int(time.time() * 1000000) + attempt * 997)
        ...
        if strategy == 0:
            ...
            dates_to_reset = [d for d in all_dates if d not in self.preferred_assignments]
            
            if dates_to_reset:
                # 每次選擇不同的日期組合
                random.shuffle(dates_to_reset)
                num_to_reset = min(5 + (attempt % 3), len(dates_to_reset) // 3)
                ...
                for date_str in selected_dates:
                    ...
                    for doctor in doctors_attending:
                        if self._check_hard_constraints(doctor, date_str, variant, local_quota, is_holiday):
            ...
            # 策略2：交換不同日期的醫師
            dates = list(variant.keys())
            random.shuffle(dates)
            
            for i in range(min(3 + (attempt % 2), len(dates)//2)):
                if i*2+1 < len(dates):
                    date1, date2 = dates[i*2], dates[i*2+1]
                    
                    # 跳過優先值班日
                    if date1 not in self.preferred_assignments and date2 not in self.preferred_assignments:
                        # 隨機決定交換主治或總醫師
                        if random.random() < 0.5:
                            variant[date1].attending, variant[date2].attending = \
                                variant[date2].attending, variant[date1].attending
                        else:
                            variant[date1].resident, variant[date2].resident = \
                                variant[date2].resident, variant[date1].resident
        
        elif strategy == 2:
            # 策略3：隨機移除一些醫師，讓其他醫師填補
            local_quota = self._calculate_used_quota(variant)
            dates = [d for d in variant.keys() if d not in self.preferred_assignments]
            random.shuffle(dates)
            
            for date_str in dates[:min(4 + (attempt % 3), len(dates)//4)]:
                slot = variant[date_str]
                is_holiday = date_str in self.holidays
                
                # 隨機移除主治或總醫師
                if random.random() < 0.5 and slot.attending:
                    quota_type = 'holiday' if is_holiday else 'weekday'
                    local_quota[slot.attending][quota_type] -= 1
                    slot.attending = None
                    
                    # 嘗試用不同的醫師填補
                    candidates = list(self.attending_doctors)
                    random.shuffle(candidates)
                    for doctor in candidates:
                        if self._check_hard_constraints(doctor, date_str, variant, local_quota, is_holiday):
                            slot.attending = doctor.name
                            local_quota[doctor.name][quota_type] += 1
                            break
            ...
            # 找出可調整的假日和平日
            adjustable_holidays = [d for d in self.holidays if d not in self.preferred_assignments]
            adjustable_weekdays = [d for d in self.weekdays if d not in self.preferred_assignments]
            
            if adjustable_holidays and adjustable_weekdays:
                random.shuffle(adjustable_holidays)
                random.shuffle(adjustable_weekdays)
                
                # 交換一些假日和平日的醫師
                for i in range(min(2 + (attempt % 2), min(len(adjustable_holidays), len(adjustable_weekdays)))):
                    h_date = adjustable_holidays[i]
                    w_date = adjustable_weekdays[i]
                    
                    # 交換主治或總醫師
                    if random.random() < 0.5:
                        variant[h_date].attending, variant[w_date].attending = \
                            variant[w_date].attending, variant[h_date].attending
                    else:
                        variant[h_date].resident, variant[w_date].resident = \
                            variant[w_date].resident, variant[h_date].resident
        
        return variant
    
    def _create_radical_variant(self, base_schedule: Dict, used_quota: Dict, attempt: int) -> Optional[Dict]:
        """創建更激進的變體（當常規方法無法產生足夠變體時）"""
        random.seed(int(time.time() * 1000000) + attempt * 1399)
        variant = copy.deepcopy(base_schedule)
        ...
        # 清空50%以上的格子
        num_to_reset = max(len(dates_to_reset) // 2, 10)
        random.shuffle(dates_to_reset)
        selected_dates = dates_to_reset[:num_to_reset]
