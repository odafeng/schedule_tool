...
class Stage1Scheduler:
        ...
        for variant_idx in range(min(5, self.constraints.beam_width)):
            ...
            while attempts < max_attempts:
                ...
                if schedule_hash not in self.generated_schedules:
                    ...
                    unique_state = self._create_state_with_variant_id(schedule, variant_idx)
                    break
    ...
    def _create_forced_variant(self, variant_idx: int) -> Optional[SchedulingState]:
        ...
        return self._create_state_with_variant_id(schedule, variant_idx)
    
    def _create_state_with_variant_id(self, schedule: Dict, variant_id: int) -> SchedulingState:
        """創建狀態並加入變體ID以確保分數差異"""
        # 計算填充數和未填格
        filled_count = 0
        unfilled_slots = []
            ...
            for role, doctor_names in roles.items():
                ...
                if len(doctor_names) > 1:
                    ...
                    if self._check_hard_constraints(
                        ...
                        preferred_failed.append(f"{date_str} {role} ({doctor_names[0]}): {reason}")
