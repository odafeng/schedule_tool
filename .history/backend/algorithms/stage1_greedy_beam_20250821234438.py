...
class Stage1Scheduler:
        ...
        for variant_idx in range(min(5, self.constraints.beam_width)):
            ...
            while attempts < max_attempts:
                ...
                if schedule_hash not in self.generated_schedules:
                    ...
                    unique_state = self._create_state_with_quota(schedule, used_quota, variant_idx)
                    break
    ...
    def _create_forced_variant(self, variant_idx: int) -> Optional[SchedulingState]:
        ...
        return self._create_state_with_quota(schedule, used_quota, variant_idx)
    ...
    def _beam_search_optimization(self, initial_states: List[SchedulingState],
        ...
        """Beam Search 優化 - 修正版：正確維護 used_quota"""
        
        # 為每個 state 建立增強版本，包含 used_quota
        enhanced_states = []
        ...
        beam = enhanced_states
        
        # 收集所有未填格（排除優先值班日）
        unfilled = []
        for state_data in beam[:1]:  # 只需要檢查一個state即可
        ...
        for step, (date_str, role) in enumerate(unfilled[:total_steps]):
            ...
            for state_data in beam:
                current_state = state_data['state']
                current_used_quota = state_data['used_quota']
                
                # 取得候選醫師（使用當前狀態的 used_quota）
                is_holiday = date_str in self.holidays
                candidates = self._get_safe_candidates(
                    date_str, role, current_state.schedule, 
                    current_used_quota, is_holiday
                )
                
                if not candidates:
                    # 無候選人，保留原狀態
                    new_beam.append(state_data)
                else:
                    # 嘗試每個候選人（最多3個）
                    for doctor_name in candidates[:3]:
                        ...
                        doctor = self.doctor_map[doctor_name]
                        if not self._check_hard_constraints(
                            doctor, date_str, new_schedule, 
                            new_used_quota, is_holiday
                        ):
                            continue
                        ...
                        # 更新 used_quota
                        quota_type = 'holiday' if is_holiday else 'weekday'
                        new_used_quota[doctor_name][quota_type] += 1
                        
                        # 創建新狀態
                        new_state = self._create_state_with_quota(
                            new_schedule, new_used_quota, 
                            len(new_beam) % beam_width
                        )
        ...
        # 返回最終的 states（不包含 used_quota）
        return [state_data['state'] for state_data in beam]
    ...
    def _create_state_with_quota(self, schedule: Dict, used_quota: Dict, 
                                 variant_id: int = 0) -> SchedulingState:
        """創建排班狀態（包含 used_quota 驗證）"""
        # 計算填充數和未填格
        filled_count = 0
        unfilled_slots = []
        ...
        # 最終驗證：確保沒有違反硬約束
        violations = self._validate_final_schedule(schedule, used_quota)
        if violations:
            # 記錄違反情況
            self.diagnostic_info['hard_constraint_violations'].extend(violations)
    ...
    def _validate_final_schedule(self, schedule: Dict, used_quota: Dict) -> List[str]:
        """最終驗證排班是否違反硬約束"""
        violations = []
        ...
        for date_str, slot in schedule.items():
            ...
            if slot.attending:
                ...
                quota_type = 'holiday' if is_holiday else 'weekday'
                max_quota = doctor.holiday_quota if is_holiday else doctor.weekday_quota
                if used_quota[slot.attending][quota_type] > max_quota:
                    violations.append(f"{slot.attending} 超過{quota_type}配額")
            ...
            if slot.resident:
                ...
                quota_type = 'holiday' if is_holiday else 'weekday'
                max_quota = doctor.holiday_quota if is_holiday else doctor.weekday_quota
                if used_quota[slot.resident][quota_type] > max_quota:
                    violations.append(f"{slot.resident} 超過{quota_type}配額")
