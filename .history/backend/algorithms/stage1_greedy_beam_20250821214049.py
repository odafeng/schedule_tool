...
class Stage1Scheduler:
    ...
    def _greedy_initialization(self) -> List[SchedulingState]:
        ...
        while len(states) < min(3, self.constraints.beam_width):
            variant = self._create_alternative_variant(schedule, used_quota, len(states))
            if variant:
                variant_state = self._create_state(variant)
                # 檢查是否真的不同
                is_different = True
                for existing in states:
                    if variant_state.score == existing.score and variant_state.filled_count == existing.filled_count:
                        is_different = False
                        break
                if is_different:
                    states.append(variant_state)
    ...
    def _create_alternative_variant(self, base_schedule: Dict, used_quota: Dict, attempt: int) -> Optional[Dict]:
        ...
        # 清空選定日期（根據 attempt 選擇不同的日期組）
        num_to_reset = min(5, len(dates_to_reset) // 4)
        start_index = (attempt * num_to_reset) % len(dates_to_reset)
        selected_dates = dates_to_reset[start_index:start_index + num_to_reset]
